...
void PAL2::TimelineScheduling(Command &req, CPDPBP &reqCPD, uint32_t pageIndex) {
  ...
  for (unsigned cur_command = 0; cur_command < erase_block; cur_command++) {
    ...
    int block = RetryNum.get_block();
    pageIndex = RetryNum.get_pageIndex();
    
    int pe = chipInfo.get_PE();
    std::string time = std::to_string(RetryNum.get_block_time(block)) + "m";
    // std::string time = chipInfo.get_Time();

    // std::cout << pageIndex<<std::endl;
    uint32_t wlIndex = pageIndex / 3;
    uint32_t wlPageIndex = pageIndex % 3;

    int retryNum = RetryNum.get_retrynum(simplessd::GetRetryNum::XSB_LIST[wlPageIndex], wlIndex, pe, time);
    ...
    // std::cout<<"pageindex: "<<pageIndex<<" wl: "<<wlIndex <<" page: "<<wlPageIndex<<" retryNum: "<<retryNum<<" latMEM: "<<latMEM<<" retrytime: "<<latMEM*(retryNum + 1)<<std::endl;
    latMEM *= (retryNum + 1);
    // latMEM *= 10;
    // retryNum+=1;
...
void PAL2::submit(Command &cmd, CPDPBP &addr, uint32_t pageIndex) {
  TimelineScheduling(cmd, addr, pageIndex);
}
