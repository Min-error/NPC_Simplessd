...
const char NAME_TLC_WL_ENABLE[] = "TLCWLEnable";
const char NAME_TLC_WL_SIZE[] = "TLCWLSize";
const char NAME_FTL_INDEX[] = "FTLINDEX";
const char NAME_COLD_DATA_THRESHOLD[] = "ColdDataThreshold";
...
Config::Config() {
  ...
  tlcWLSize = 3;
  ftlIndex = 1;
  coldDataThreshold = 131072;  // 128KB
}
...
bool Config::setConfig(const char *name, const char *value) {
  ...
  else if (MATCH_NAME(NAME_TLC_WL_ENABLE)) {
    tlcWLEnable = convertBool(value);
  }
  else if (MATCH_NAME(NAME_TLC_WL_SIZE)) {
    tlcWLSize = strtoul(value, nullptr, 10);
  }
  else if (MATCH_NAME(NAME_FTL_INDEX)) {
    ftlIndex = strtoul(value, nullptr, 10);
  }
  else if (MATCH_NAME(NAME_COLD_DATA_THRESHOLD)) {
    coldDataThreshold = strtoul(value, nullptr, 10);
  }
  else {
    ret = false;
  }
...
uint64_t Config::readUint(uint32_t idx) {
  ...
  switch (idx) {
    ...
    case FTL_GC_D_CHOICE_PARAM:
      ret = dChoiceParam;
      break;
    case FTL_TLC_WL_SIZE:
      ret = tlcWLSize;
      break;
    case FTL_INDEX:
      ret = ftlIndex;
      break;
    case FTL_COLD_DATA_THRESHOLD:
      ret = coldDataThreshold;
      break;
  }
