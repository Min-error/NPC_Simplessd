...
PageMapping::PageMapping(ConfigReader &c, Parameter &p, PAL::PAL *l,
  ...
  // uint32_t ftlIndex = conf.readInt(CONFIG_FTL, FTL_INDEX);
  // uint32_t testNumber = ftlIndex % 10;  // 获取最后一位数字
  
  // std::string file_name1 = "data/write_time" + std::to_string(testNumber) + ".csv";
  // debugprint(LOG_FTL_PAGE_MAPPING, 
  //            "FTL Index: %u, Test Number: %u, Output file: %s", 
  //            ftlIndex, testNumber, file_name1.c_str());
  // outfile1.open(file_name1, std::ios::out);
  // if (!outfile1.is_open()) {
  //   std::cout << file_name1 << std::endl;
  //   std::cout << "Error: opening file fail" << std::endl;
  //   std::exit(1);
  // }
  // outfile1 << "lpn,endTick,beginTick,writeTime"
  //          << std::endl;

  uint32_t ftlIndex = conf.readInt(CONFIG_FTL, FTL_INDEX);
  uint32_t testNumber = ftlIndex;  // 获取最后一位数字
  std::string readFileName = "data_info/msr/read_info" + std::to_string(testNumber) + ".csv";

  debugprint(LOG_FTL_PAGE_MAPPING, 
             "FTL Index: %u, Test Number: %u, Output file: %s", 
             ftlIndex, testNumber, readFileName.c_str());
  ...
  readTimeFile << "block,wl,page,pageindexs"
           << std::endl;

  // 初始化WL缓存：8个元素的vector数组
  wlCache.resize(8);
  for (uint32_t i = 0; i < 8; i++) {
    wlCache[i].clear();
  }

  // 初始化lineCountInSuperPage
  lineCountInSuperPage = param.ioUnitInPage;
}

PageMapping::~PageMapping() {
  // 在析构函数中不处理WL缓存，因为依赖的对象可能已经被销毁
  // 只记录剩余的缓存信息到日志文件
  for (uint32_t i = 0; i < 8; i++) {
    if (!wlCache[i].empty()) {
      debugprint(LOG_FTL_PAGE_MAPPING, 
                 "Warning: %zu elements remaining in WL cache %u at destruction", 
                 wlCache[i].size(), i);
      wlCache[i].clear();
    }
  }
...
void PageMapping::read(Request &req, uint64_t &tick) {
  ...
  if (req.ioFlag.count() > 0) {
    if (req.isWLGranularity) {
      // WL粒度读取
      readWLInternal(req, tick);
      debugprint(LOG_FTL_PAGE_MAPPING,
                 "READ  | LPN %" PRIu64 " | WL:YES | %" PRIu64 " - %" PRIu64 " (%" PRIu64
                 ")",
      ...
      debugprint(LOG_FTL_PAGE_MAPPING,
                 "READ  | LPN %" PRIu64 " | WL:NO | %" PRIu64 " - %" PRIu64 " (%" PRIu64
                 ")",
                 req.lpn, begin, tick, tick - begin);
    }
  }
...
void PageMapping::write(Request &req, uint64_t &tick) {
  ...
  if (req.ioFlag.count() > 0) {
    // 将请求添加到对应的WL缓存中
    wlCache[pSetAt].push_back(req);
    
    debugprint(LOG_FTL_PAGE_MAPPING,
               "WRITE | LPN %" PRIu64 " | pSetAt %u | Cache size %zu | %" PRIu64 " - %" PRIu64 " (%" PRIu64 ")",
               req.lpn, pSetAt, wlCache[pSetAt].size(), begin, tick, tick - begin);
    
    // 检查是否需要flush WL缓存（TLC需要3个page组成一个WL）
    flushWLCache(pSetAt, tick);
  }
  else {
    warn("FTL got empty request");
  }
...
void PageMapping::readInternal(Request &req, uint64_t &tick) {
  ...
  // uint32_t wlPageIndex = req.lpn % 3;
  ...
  if (mappingList != table.end()) {
    ...
    // std::cout<<"page_mapping begin: "<<tick<<std::endl;
    for (uint32_t idx = 0; idx < bitsetSize; idx++) {
      if (req.ioFlag.test(idx) || !bRandomTweak) {
        ...
        if (mapping.first < param.totalPhysicalBlocks &&
          ...
          if (readTimeFile.is_open()) {
            readTimeFile << palRequest.blockIndex << ","
                         << palRequest.pageIndex / 3 << ","
                         << palRequest.pageIndex %3 << ","
                         << palRequest.pageIndex << std::endl;
          }
          // std::cout<<"block: "<<palRequest.blockIndex<<" wl: "<<palRequest.pageIndex/3<<" page: "<<palRequest.pageIndex%3<<" page index: "<<palRequest.pageIndex<<std::endl;
          
            uint32_t pageIndex = palRequest.pageIndex;
            uint32_t blockIndex = palRequest.blockIndex;
            auto& RetryNum = simplessd::GetRetryNum::getInstance();
            RetryNum.set_pageIndex(pageIndex);
            RetryNum.set_block(blockIndex);
          if (bRandomTweak) {
          ...
          beginAt = tick;
          // std::cout<<"1 "<<palRequest.pageIndex<<" "<<req.lpn<<std::endl;    
    ...
    // std::cout<<"page_mapping end: "<<finishedAt<<" spend time: "<<finishedAt - tick<<std::endl;
...
void PageMapping::flushWLCache(uint32_t psetAt, uint64_t &tick) {
  // 检查指定psetAt的WL缓存是否有3个元素
  if (wlCache[psetAt].size() >= 3) {
    debugprint(LOG_FTL_PAGE_MAPPING, 
               "Flushing WL cache for psetAt %u with %zu elements", 
               psetAt, wlCache[psetAt].size());
    
    uint64_t startTick = tick;
    uint64_t finished = tick;
    uint64_t begin = tick;
    
    // 对缓存中的3个元素依次调用writeInternal，但时间计算方式改为并行
    for (size_t i = 0; i < 3; i++) {
      Request &cachedReq = wlCache[psetAt][i];
      
      begin = tick;  // 每个page都从相同的tick开始
      writeInternal(cachedReq, begin, true);
      finished = std::max(finished, begin);  // 取最大的完成时间

      // outfile1 << cachedReq.lpn << "," << begin << "," << tick <<"," <<begin - tick<<std::endl;
    }
    
    // 整个WL的完成时间设为所有page中最大的完成时间
    tick = finished;
    
    // 移除已处理的3个元素，保留剩余元素
    wlCache[psetAt].erase(wlCache[psetAt].begin(), wlCache[psetAt].begin() + 3);
    
    debugprint(LOG_FTL_PAGE_MAPPING, 
               "WL cache flushed for psetAt %u, remaining %zu elements, WL time: %" PRIu64, 
               psetAt, wlCache[psetAt].size(), tick - startTick);
  }
}

void PageMapping::flushAllWLCache(uint64_t &tick) {
  // 强制flush所有WL缓存，即使不足3个元素
  for (uint32_t i = 0; i < 8; i++) {
    if (!wlCache[i].empty()) {
      debugprint(LOG_FTL_PAGE_MAPPING, 
                 "Force flushing %zu elements in WL cache %u", 
                 wlCache[i].size(), i);
      ...
      // 处理所有剩余的元素
      for (auto &cachedReq : wlCache[i]) {
        begin = tick;
        writeInternal(cachedReq, begin, true);
        finished = std::max(finished, begin);
      ...
      debugprint(LOG_FTL_PAGE_MAPPING, 
                 "Force flushed WL cache %u, time: %" PRIu64, 
                 i, tick - startTick);
    }
  }
}
...
void PageMapping::readWLInternal(Request &req, uint64_t &tick) {
  PAL::Request palRequest(req);
  uint64_t beginAt;
  uint64_t finishedAt = tick;
  
  // 计算当前page在WL中的位置 (0, 1, 2)
  uint32_t wlPageIndex = req.lpn % 3;

  // 计算WL的起始LPN
  uint64_t wlStartLPN = req.lpn - wlPageIndex;
  
  debugprint(LOG_FTL_PAGE_MAPPING,
             "READ_WL | LPN %" PRIu64 " | WL Start LPN %" PRIu64 " | Page Index %u",
             req.lpn, wlStartLPN, wlPageIndex);
  
  // 如果是WL的第一个page，读取整个WL (3个page)
  if (wlPageIndex == 0) {
    for (uint32_t i = 0; i < 3; i++) {
      uint64_t currentLPN = wlStartLPN + i;
      auto mappingList = table.find(currentLPN);
    ...
    debugprint(LOG_FTL_PAGE_MAPPING,
               "READ_WL | Completed WL read for LPNs %" PRIu64 "-%" PRIu64,
               wlStartLPN, wlStartLPN + 2);
  } else {
    // 如果不是WL的第一个page，可能已经被读取过了
    // 这里可以添加缓存检查逻辑，或者简单地记录日志
    debugprint(LOG_FTL_PAGE_MAPPING,
               "READ_WL | Page %u in WL already read in previous WL operation",
               wlPageIndex);
  }
  
  tick = finishedAt;
  tick += applyLatency(CPU::FTL__PAGE_MAPPING, CPU::READ_WL_INTERNAL);
}
