...
bool GenericCache::read(Request &req, uint64_t &tick) {
  ...
  if (useReadCaching) {
    ...
    ICL_GENERIC_CACHE_READ:
      ...
      // std::cout<<"generic_cache begin: "<<tick<<std::endl;
      ...
      for (auto &iter : readList) {
        ...
        // std::cout<<req.range.slpn<<" "<<reqInternal.lpn<<std::endl;
      ...
      // std::cout<<"generic_cache end: "<<finishedAt<<" spend time: "<<finishedAt - tick<<std::endl;
      tick = finishedAt;
    ...
    std::cout<<"generic_cache end: "<<tick<<" spend time: "<<tick - ttick<<std::endl;
  }
