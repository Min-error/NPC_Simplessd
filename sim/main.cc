...
std::string chipName = "3DV7";
// std::string chipName = "X3_9070";
std::string retryNumPath = "get_retrynum/";
std::string modelType = "_not_group";
...
int main(int argc, char *argv[]) {
  ...
  if (argc != 8) {
    ...
    std::cerr << "  Usage: simplessd-standalone <Simulation configuration "
                 "file> <SimpleSSD configuration file> <Output directory> <chip_name> <model_name> <PE> <Time>"
              << std::endl;
    std::cerr << "  Example: simplessd-standalone config/sample.cfg simplessd/config/sample.cfg result micron_1000lt MLC 3000 1m"
              << std::endl;
  ...
  chipName = argv[4];
  modelType = argv[5];
  int ppe = std::stoi(argv[6]);
  std::string ttime = argv[7];

  auto& chipInfo = simplessd::ChipInfo::getInstance();
  chipInfo.set_info(chipName);
  chipInfo.set_ModelType(modelType);
  chipInfo.set_PE(ppe);
  // chipInfo.set_Time(ttime);
  auto& retryNum = simplessd::GetRetryNum::getInstance();
  retryNum.read_all_retrynum(retryNumPath + "retrynum/" + chipName + "/", modelType);
  retryNum.read_all_blockPE(retryNumPath + "block_pe/");

  std::cout << "Running simulation with parameters:" << std::endl;
  std::cout << "Chip: " << chipName << std::endl;
  std::cout << "Model: " << modelType << std::endl;
  std::cout << "PE: " << ppe << std::endl;
